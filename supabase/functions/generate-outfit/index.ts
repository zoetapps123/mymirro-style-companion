...
serve(async (req) => {
  ...
  try {
    const { generationType, occasion, style, anchorItem, wardrobeItems, maxOutfits, userLocation } = await req.json();
    ...
    if (cachedOutfits) {
      ...
      return new Response(JSON.stringify({ success: true, outfits: cachedOutfits }), {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a professional fashion stylist. You MUST call the generate_outfit_combinations function with properly structured JSON data. Do not return plain text or markdown - only use the function call.",
        },
        {
          role: "user",
          content: prompt,
    ...
    if (!result?.outfits || !Array.isArray(result.outfits) || result.outfits.length === 0) {
      ...
      return new Response(JSON.stringify({ success: true, outfits: [] }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const outfitsWithItems = result.outfits.map((outfit: any) => {
      const outfitItems = outfit.pieces
        .map((piece: any) => wardrobeItems.find((item: any) => item.id === piece.wardrobeItemId))
        .filter(Boolean);
